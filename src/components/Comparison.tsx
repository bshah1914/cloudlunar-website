...
function CellValue({ value }: { value: boolean | string }) {
  ...
  return <span className="text-sm text-gray-400 dark:text-gray-500 font-medium">{value}</span>;
}

export default function Comparison() {
  return (
    <section className="relative py-28 md:py-36">
      <div className="absolute inset-0 bg-gradient-to-b from-white dark:from-[#030712] via-blue-50/20 dark:via-transparent to-white dark:to-[#030712] pointer-events-none" />
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="glass-card overflow-x-auto">
          <div className="grid grid-cols-4 gap-4 px-6 py-4 border-b border-gray-200 dark:border-white/10 bg-white/70 dark:bg-white/5">
            <div className="text-sm text-gray-500 dark:text-gray-400 font-medium">Feature</div>
            <div className="text-center">
              <span className="text-sm font-bold text-gradient">CloudLunar</span>
            </div>
            <div className="text-center text-sm text-gray-500 dark:text-gray-400">Competitor A</div>
            <div className="text-center text-sm text-gray-500 dark:text-gray-400">Competitor B</div>
          </div>

          {categories.map((cat) => (
            <div key={cat.name}>
              <div className="px-6 py-3 bg-white/70 dark:bg-white/5 border-b border-gray-100 dark:border-white/5">
                <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{cat.name}</span>
              </div>
              {cat.features.map((row) => (
                <div key={row.feature} className="grid grid-cols-4 gap-4 px-6 py-3 border-b border-gray-100 dark:border-white/5 hover:bg-white/70 dark:hover:bg-white/5 transition-colors">
                  <div className="text-sm text-gray-500 dark:text-gray-400">{row.feature}</div>
